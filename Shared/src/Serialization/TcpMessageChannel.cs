...
namespace Shared.Serialization
{
    ...
    public class TcpMessageChannel
    {
        ...
        {
            LogUtil.LogInfo("TCPMessageChannel created around "+pTcpClient, this, ConsoleColor.Blue);
        ...
        {
            LogUtil.LogInfo("TCPMessageChannel created (not connected).", this, ConsoleColor.Blue);
        }
        ...
        {
            LogUtil.LogInfo("Connecting...", this, ConsoleColor.Blue);
            ...
            try
            {
                ...
                _remoteEndPoint = _client.Client.RemoteEndPoint as IPEndPoint;
                _errors.Clear();
                LogUtil.LogInfo("Connected.", this, ConsoleColor.Blue);
                return true;
            }
            catch (Exception e)
            {
        ...
        public void SendMessage(ASerializable pMessage)
        {
            if (HasErrors())
            {
                LogUtil.LogInfo("This channel has errors, cannot send.", this, ConsoleColor.Red);
                return;
            }

            //everything we log from now to the end of this method should be cyan
            LogUtil.PushForegroundColor(ConsoleColor.Cyan);
            LogUtil.LogInfo(pMessage, this);
        ...
        public ASerializable ReceiveMessage()
        {
            if (HasErrors())
            {
                LogUtil.LogInfo("This channel has errors, cannot receive.", this, ConsoleColor.Red);
                return null;
            }

            try
            {
                LogUtil.PushForegroundColor(ConsoleColor.Yellow);
                LogUtil.LogInfo("Receiving message...", this);
                
                byte[] inBytes = StreamUtil.Read(_stream);
                Packet inPacket = new Packet(inBytes);
                ASerializable inObject = inPacket.ReadObject();
                LogUtil.LogInfo("Received " + inObject, this);
                LogUtil.PopForegroundColor();
        ...
        {
            LogUtil.LogInfo("Error added:"+pError, this, ConsoleColor.Red);
            _errors.Add(pError);
            Close();
        }
