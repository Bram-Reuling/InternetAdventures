...
namespace Networking
{
    ...
    public class NetworkCharacterMovement : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            //Setup input
            //Movement
            _playerInput.actions.FindAction("Movement").performed += OnMovePressed;
            _playerInput.actions.FindAction("Movement").canceled += OnMoveReleased;
            //Jump
        ...
        private void Update()
        {
            ...
            if (_velocity.magnitude > Single.Epsilon)
            {
                // Send PlayerMoveRequest packets to replicate movement (TEMP FIX)
                var characterTransform = transform;
                PlayerMoveRequest playerMoveRequest = new PlayerMoveRequest
                    {inputPosition = characterTransform.position, inputRotation = characterTransform.rotation};
                
                networkManager.SendObject(playerMoveRequest);
            }
        }

        // TODO: Convert stuffs to networked stuffs
        private void OnMovePressed(InputAction.CallbackContext pInputValue)
        {
            //Read in new vector
        ...
        private void OnMoveReleased(InputAction.CallbackContext pInputValue)
        {
